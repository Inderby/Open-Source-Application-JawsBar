...
public:
  void SetUp() override;
  int Perform(std::pair<std::string, int>);
...
static std::pair<std::vector<std::pair<std::string, int>>, std::vector<int>>
ReadTestCases(const std::string &filename_input,
  ...
  if (file_input.is_open() && file_output.is_open()) {
    int number_of_commmand = 0, value = 0, result = 0;
    std::string command;
...
int AVLTreeFixture::Perform(std::pair<std::string, int> command_and_value) {
  std::string command = command_and_value.first;
  int value = command_and_value.second;
  int return_value = -1;
  if (command == "insert") {
    return_value = avl_tree_.Insert(value);
  } else if (command == "empty") {
    return_value = avl_tree_.Empty();
  } else if (command == "erase") {
    return_value = avl_tree_.Erase(value);
  }
  return return_value;
}
